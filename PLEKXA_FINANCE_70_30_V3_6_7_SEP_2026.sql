-- PLEKXA Finance 70/30 policy — corrected migration v3.6.7
...
CREATE OR REPLACE FUNCTION public.plekxa_recalculate_index_period(
  p_index_id uuid,
  p_start date,
  p_end date,
  p_currency char(3)
...
BEGIN
  ...
  LOOP
    FOR c IN
      SELECT id, contributor_name, master_share
    ...
    LOOP
      v_amount := round(
        ...
        * coalesce(c.master_share, 0) / 100.0,
        2
      );
      ...
      VALUES (
        ...
        coalesce(c.master_share, 0),
        coalesce(a.index_participation_percentage, 0) * coalesce(c.master_share, 0) / 100.0,
        v_index_pool,
        v_amount,
        p_currency,
        'calculated'
