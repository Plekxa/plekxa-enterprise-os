...
type SidebarProps={mobileOpen?:boolean;onClose?:()=>void;access:{role:string;permissions:string[];isMaster?:boolean}|null};
...
export default function Sidebar({mobileOpen=false,onClose,access}:SidebarProps){
 ...
 const isMaster=Boolean(access?.isMaster)||role.trim().toLowerCase().replace(/[\s_-]+/g,' ')==='super admin';
 let section='';
 return <>
